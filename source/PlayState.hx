		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				...
				case 'lamplight-town':
					{
					defaultCamZoom = 0.70;
					curStage = 'lamplight-town';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('skyProdigy'));
					bg.scrollFactor.set(0.9, 0.9);
					add(bg);

					var borderBG:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('borderWall'));
					borderBG.scrollFactor.set(0.9, 0.9);
					add(borderBG);

					var plantsBG:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('plants_BG'));
					plantsBG.scrollFactor.set(0.9, 0.9);
					add(plantsBG);

					var tileBG:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('tileFloor'));
					tileBG.scrollFactor.set(0.9, 0.9);
					add(tileBG);
